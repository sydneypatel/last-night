...
router.get('/', auth, async (req, res, next) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT p.*, pl.saved_at, u.username AS author_username, u.display_name AS author_display_name, g.name AS group_name FROM personal_library pl JOIN photos p ON p.id = pl.photo_id JOIN users u ON u.id = p.user_id JOIN groups g ON g.id = p.group_id WHERE pl.user_id = $1 ORDER BY pl.saved_at DESC`,
      [req.user.id]
    );
